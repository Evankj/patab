...
class PatternAbstractor:
    ...
    def get_matches(self) -> List[Dict]:
        ...
        """
        rg = Ripgrepy(self.pattern, str(self.directory)).i().with_filename().line_number().no_heading().glob(f'**/*.{self.extension}').json()
        results = rg.run().as_dict
        return results

    def write_matches_to_template(self, matches: List[Dict], template: Template, output_file: PathLike) -> bool:
        """
